...
function Bulk(action, client, index, type, options) {
  ...
  if (!client) throw "CLIENT_MISSING";

  if (!action) throw "ACTION_MISSING";
...
Bulk.prototype._fn = function (d) {
  ...
  let itemsToProcess = [].concat(d).reduce((p, d) => {
    ...
    if (this.action == "index") p.push(d);
    else if (this.action == "upsert") p.push({ doc: d, doc_as_upsert: true });
    else if (this.action == "update") p.push({ doc: d });
    return p;
  }, []);
  ...
  const processError = (e) => {
    ...
    if (this.options.debug) console.log("Retry", e.message);
  ...
  const execute = () => {
    return this.client
      ...
      .then(
        (e) => {
          if (!this.options.pushResult && !this.options.pushErrors) return;
          ...
          if (this.options.maxRetries) {
            ...
            e.items.forEach((item, index) => {
              ...
              if (
                verb &&
                verb.error &&
                verb.error.type !== "mapper_parsing_exception"
              ) {
          ...
          if (this.options.pushResults) return e;
